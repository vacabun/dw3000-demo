 ...
 *           example "ACK DATA TX"). The DW IC is configured so that when a correctly addressed data frame is received with the ACK request (AR) bit
...
int ack_data_rx(void)
{
    ...
    while (1)
    {
        ...
        /* Poll until a frame is properly received or an RX error occurs. See NOTE 6 below.
